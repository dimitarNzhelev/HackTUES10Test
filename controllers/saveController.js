...
async function savePost(userid, postid){
    try{
        const result = await pool.query('INSERT INTO saved_posts(user_id, post_id) VALUES($1, $2)', [userid, postid]);
        return result;
    }catch(err){
...
async function unsavePost(userid, postid){
    try{
        const result = await pool.query('DELETE FROM saved_posts WHERE user_id = $1 AND post_id = $2', [userid, postid]);
        return result;
    }catch(err){
        throw err;
    }
...
async function checkSavedStatus(userid, postid){
    try{
        const result = await pool.query('SELECT * FROM saved_posts WHERE user_id = $1 AND post_id = $2', [userid, postid]);
